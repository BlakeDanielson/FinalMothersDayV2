...
async function auditRecipeCategories(): Promise<CategoryAuditResult> {
  ...
  try {
    ...
    const categoryUserCounts = await Promise.all(
      categoryStats.map(async (stat: { category: string; _count: { category: number; userId: number } }) => {
    ...
    if (uiOnly.length > 0) {
      recommendations.push(`⚠️  UI includes categories not in UI prompt: ${uiOnly.join(', ')}`);
    }
